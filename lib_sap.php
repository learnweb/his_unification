...
define("SAP_PERSONAL_LOGIN",      "public.ovv_lehrende_email");
...
function get_teachers_pid_sap($username) {
    global $pgDB;
    $q = pg_query($pgDB->connection,
            "SELECT sapid FROM " . SAP_PERSONAL_LOGIN . " WHERE (login = '" . $username . "')");
...
function get_veranstids_by_teacher_sap($pid) {
    global $pgDB;
    $q = pg_query($pgDB->connection,
            "SELECT v.objid_klvl FROM " . SAP_GRUPPE_P . " as g Join " . SAP_GRUPPE_V .
            " as v on v.objid = g.objid and v.otype = g.otype WHERE sapid = $pid
            group by v.objid_klvl order by v.objid_klvl;");
    $return = array();
    while ($veranstid = pg_fetch_object($q)) {
        array_push($return, $veranstid->objid_klvl);
    }
    return $return;
}
