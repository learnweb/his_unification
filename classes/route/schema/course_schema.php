...
/**
 * Schema of a single course. Must be equal to the course_dto.
 *
 * Shared by the responses that return courses and by the request bodies that accept one,
 * so that a change to the dto only has to be made here.
 *
 * @package   local_lsf_unification
 * @copyright 2026 Tamaro Walter
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class course_schema extends schema_object {
    /**
     * Constructor.
     *
    ...
    public function __construct(bool $required = false) {
        parent::__construct(
            content: [
                ...
                'cms' => new scalar_type(param::RAW, required: $required),
                'cmsinstance' => new scalar_type(param::INT, required: $required),
                'cmsurl' => new scalar_type(param::RAW, required: $required),
                'title' => new scalar_type(param::RAW, required: $required),
                'shorttitle' => new scalar_type(param::RAW, required: $required),
                'description' => new scalar_type(param::RAW, required: $required),
                'teacher' => new scalar_type(param::RAW, required: $required),
                'semester' => new scalar_type(param::RAW, required: $required),
                'created' => new scalar_type(param::INT, required: $required),
                'state' => new scalar_type(param::INT, required: $required),
                'courseid' => new scalar_type(param::INT, required: $required),
                'moodleid' => new scalar_type(param::INT, required: $required),
                'requeststate' => new scalar_type(param::INT, required: $required),
            ],
        );
    }
